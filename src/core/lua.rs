...
impl LuaScriptRunner {
    ...
    pub fn init_environment(&self, host: &str, port: Option<u16>) -> LuaResult<()> {
        ...
        let connect_fn = self.lua.create_function(move |_, (host, port): (String, u16)| {
    ...
    pub async fn run_script(&self, script_name: &str, host: &str, port: Option<u16>) -> Result<ScriptResult, Box<dyn Error>> {
    ...
    pub async fn run_scripts(&self, script_names: &[String], host: &str, port: Option<u16>) -> Vec<ScriptResult> {
    ...
    pub async fn run_all_scripts(&self, host: &str, port: Option<u16>) -> Result<Vec<ScriptResult>, Box<dyn Error>> {
